...
from .models import CompanyRelations,EmployeeDocuments,Attendence,Breaks,DailyAttendanceReport,TodaysEmployeeActivity
from django.db.models import Sum,F,ExpressionWrapper,DurationField
from .serializers import EmployeeRelationSerializer,DocumentsSerializer,AttendanceSerializer,LogsSerializer,AllLogsSerializer,EmployeeDailyActivitiesSerializer,EmployeeProfileSerializer,DocumentsUpdateSerializer,RelationsUpdateSerializer
...
class DailyAttendanceReportEmployee(viewsets.ModelViewSet):
    ...
    def get_queryset(self):  
          ...
          if self.request.user.is_superuser:
               if id:
                   return DailyAttendanceReport.objects.filter(id=id)
            ...
            #    attendance = DailyAttendanceReport.objects.select_related('employee_id').all()  
               attendance = DailyAttendanceReport.objects.filter(employee_id = self.request.user.id)           
               return  attendance
          else:
            if start_date and end_date:
                return DailyAttendanceReport.objects.filter(employee_id = self.request.user.id,date__range=[start_date, end_date])
            else:
                return DailyAttendanceReport.objects.filter(employee_id = self.request.user.id)
    def get_permissions(self):
        if self.request.method in ["POST","PUT","PATCH","DELETE"]:
            self.permission_classes = [permissions.IsAdminUser]
        else:
...
class BreakIn(APIView):
    ...
    def post(self,request):
            ...
            if employee is None:
                ...
                print(employee,"break IN details h bhai")  
